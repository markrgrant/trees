...
CREATE OR REPLACE FUNCTION root_container_al()
    RETURNS TABLE(id int, name text, parent_id int) AS $$
    BEGIN
        RETURN QUERY
...
CREATE OR REPLACE FUNCTION descendant_containers_al(_id int)
    ...
    BEGIN
        RETURN QUERY
        WITH RECURSIVE descendant_containers(id, name, parent_id) AS (
...
CREATE OR REPLACE FUNCTION create_container_al(
    ...
    BEGIN
        ...
        ELSE
            ...
            BEGIN
                INSERT INTO
                    container_al(name, parent_id)
                VALUES
                    (gen_random_uuid()::text, parent_id)
                RETURNING
                    id
                INTO parent_id_val;
